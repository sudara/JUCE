...
ComponentPeer* Component::createNewPeer (int styleFlags, void* nativeWindowToAttachTo)
{
    if (isUsingWaylandBackend())
    {
        ...
        auto* parentSurface = static_cast<wl_surface*> (nativeWindowToAttachTo);

        // A non-null handle may still name an X11 parent during a Wayland session.
        if ((styleFlags & ComponentPeer::windowIsTemporary) != 0)
            if (auto* peer = createWaylandComponentPeer (*this, styleFlags, parentSurface))
                return peer;
    }
...
public:
    NativeDarkModeChangeDetectorImpl()
    {
        if (isUsingWaylandBackend())
        {
            // Avoid eager XSettings/X11 startup because native dark-mode support is deferred.
            return;
        }
    ...
    ~NativeDarkModeChangeDetectorImpl() override
    {
        if (auto* windowSystem = WaylandWindowSystem::getInstanceWithoutCreating())
            ...
            {
                // No XSettings listener was registered on the Wayland path.
                return;
            }
